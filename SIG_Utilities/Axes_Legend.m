...
y_marker_length = round(50*(y_length / y_divider))/50;
x_marker_length = round(500*(x_length / x_divider))/500;
...
x_text_y_position = y_lims(1) - (y_length * 0.03);
text(x_text_x_position, x_text_y_position, x_text, 'FontSize', 20, ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');

% Y-axis text
y_text = strcat(string(y_marker_length), {'  '}, y_units);
y_text_x_position = x_lims(2) + (x_length * 0.01);
y_text_y_position = y_lims(1) + (y_marker_length / 2);
y_axis = text(y_text_x_position, y_text_y_position, y_text, 'FontSize', 20, ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
y_axis.Rotation = 90;
