function [peaktopeak_MEP] = Avg_StartMEP(sig, State, muscle_group, Save_Figs)
...
[per_trial_EMG, EMG_Names] = Trial_StartMEP(sig, State, muscle_group, 0, 0);
...
avg_MEP = struct([]);
for ii = 1:length(per_trial_EMG)
    for mm = 1:height(per_trial_EMG{ii,1})
        avg_MEP{ii,1}(mm,1) = mean(per_trial_EMG{ii,1}(mm,:));
    end
end

peaktopeak_MEP = struct([]);
for ii = 1:length(per_trial_EMG)
    for mm = 1:width(per_trial_EMG{ii,1})
        peaktopeak_MEP{ii,1}(mm,1) = peak2peak(per_trial_EMG{ii,1}(:,mm));
    end
end
...
for ii = 1:length(EMG_Names)
    ...
    plot(absolute_timing(start_idx:stop_idx), avg_MEP{ii}, ...
        'LineWidth', 2, 'Color', 'k')
